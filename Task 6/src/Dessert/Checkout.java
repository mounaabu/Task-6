...
public class Checkout {
	...
	private final int RECEIPT_WIDTH = 50;
	...
	public String toString() {
		...
		for(int j = 0; j < numberOfItems; j++){
			...
			if (p.length() > DessertShoppe.COST_WIDTH)		// verify width
				p = p.substring(0, DessertShoppe.COST_WIDTH);
			
			if (myDessertItems[j] instanceof IceCream) {		//if ice cream
				while(l.length() < RECEIPT_WIDTH - p.length()){
					l += " ";
				}
				s += l + p + "\n";
			}
			else if (myDessertItems[j] instanceof Sundae) {		//if Sundae
			...
			else if (myDessertItems[j] instanceof Candy){		//if Candy
				s += ((Candy) myDessertItems[j]).getweight() + " pound" + 
						DessertShoppe.cents2dollarsAndCents(((Candy) myDessertItems[j]).getppp()) + " /lb.\n";
				...
				s += ((Cookie)myDessertItems[j]).getnumber() + " @ " + 
						DessertShoppe.cents2dollarsAndCents(((Cookie)myDessertItems[j]).getppd()) + "dozen";
